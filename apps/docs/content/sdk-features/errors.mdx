...
See the [custom error capture example](https://github.com/tldraw/tldraw/tree/main/apps/examples/src/examples/ui/custom-error-capture) for a custom `ErrorFallback`, and the [error boundary example](https://github.com/tldraw/tldraw/tree/main/apps/examples/src/examples/ui/error-boundary) for a custom `ShapeErrorFallback`.
...
See the [crash event handling example](https://github.com/tldraw/tldraw/tree/main/apps/examples/src/examples/ui/crash-event-handling) for a working demo.
...
Annotations include `tags` (key-value pairs for categorization) and `extras` (additional context data). Add your own with `annotateError` before letting an error propagate to a boundary:

```tsx
import { annotateError } from 'tldraw'

try {
	riskyOperation()
} catch (error) {
	annotateError(error, {
		tags: { origin: 'my-feature' },
		extras: { documentId },
	})
	throw error
}
...
| `annotateError(error, annotations)` | Attach tags and extras to an error object            |
...
- **[Crash event handling](https://github.com/tldraw/tldraw/tree/main/apps/examples/src/examples/ui/crash-event-handling)** — Listen for the editor's crash event and report fatal errors to an error tracking service.
