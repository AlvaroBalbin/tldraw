...
Every shape has a `meta` property for your own custom data. Tldraw never reads this data itself. It's an escape hatch for attaching extra information to shapes, like the name of the user who created a shape or the date it was last changed.

Because `meta` is part of the shape record, your data behaves like the rest of the shape: it's included in undo and redo, copied and pasted along with the shape, saved and loaded with the document, and synced to the other users in a [multiplayer](/docs/sync) room.
